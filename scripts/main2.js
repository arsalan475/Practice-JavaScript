...
let id = localStorage.getItem('documentId');
    ...
    if (calendarViewConfig.fullYearMode) {
      calendar.months.forEach(month => {
        ...
        renderMonth(month, container,calendarViewConfig.fullYearMode);
      });
...
function renderMonth(monthObj, container,fullYearMode) {
  ...
  const streak = getDataFromServer('streak')
    ...
    if(fullYearMode){
      ...
      document.getElementById('month-name').innerHTML =  `<div>${monthObj.name} </br> <span> ${data?.content}</span></div>`
    }
